...
def _render_heart_form() -> dict:
    """Render heart form controls driven by manifest field_specs."""
    ...
    counts = _split_counts(len(SPECS), 3)
    cursor = 0
        ...
        try:
            ...
            save_case_context(
                CaseContext(
                    ...
                    summary=summary,
                    not_seen=(
                        "the physical examination, full history, the ECG "
                        "tracing itself, and any labs beyond those entered above"
                    ),
                )
            )
